...
Create `article-<kebab-slug>.html` by copying the newest existing deep-dive (**`article-visionpro-surgery.html`**) as the template and rewriting the content. Keep every `<style>` rule that already exists id-to-identical — do not redesign.
